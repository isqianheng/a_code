...
<style type="text/css">
    .Galaxy-headerMid .menuList:first-child {margin-left:10px;}
	.Galaxy-headerMid .menuList	{margin-left:10px;}
	.firstMenu {line-height:40px;padding-left:46px;font-size:14px;border-bottom:1px solid #f0f0f0;}
	.firstMenu:hover {background-color:#f2f2f2;}
	.menu_dropdown > ul > li > a.current {background-color:#d9d9d9;}
</style>

<body class="big-page">
<header class="Galaxy-header cl">
	<div class="logoArea" align="center">
    	<a class="Galaxy-logo l" title="DCITS">交易辅助系统</a>
    	<span class="Galaxy-subtitle l"></span>
	</div>
	<div class="header-top">
		<nav class="mainnav cl" id="Galaxy-nav">
			<ul class="Galaxy-headerMid" id="firstMenu">
			</ul>
			<ul class="Galaxy-userbar_narrow">
				<li class="dropDown dropDown_hover"><a href="#" class="dropDown_G"><i class="iconfont" >&#xe633;</i>&nbsp;${UserName}</a>
						<ul class="dropDown-menu radius box-shadow">
							<!--<li><a href="#" onclick="changePassword('修改个人密码','changePassword.jsp','600','350')" ><i class="iconfont">&#xe620;</i> 修改个人密码</a></li>-->
		...
		success:function(json){
			if(json){
				...
				for(i=0;i<json.length;i++){
					firstMenu += "<li class='menuList'><a href='javascript:void(0)' id='"+json[i].id.split(",")[2]+"' onClick='displayMenu(this)' title='"+json[i].id.split(",")[1]+ "'><i class='iconfont' style='color:#fff;font-size:15px;margin-right:10px;'>"+json[i].id.split(",")[0]+"</i>"+json[i].id.split(",")[1]+"</a></li>";
					if(json[i].children){
					    $("#asideMenu").data(json[i].id.split(",")[2],json[i].children);
					}
				}
				$("#firstMenu").append(firstMenu);
			}
		}
	});
	/*左侧菜单*/
